    ...
    <script>
        ...
        function doInit(){
            $('#table-bootstrap').bootstrapTable({
                ...
                url: "/shareChannel/getShareChannelList",
        ...
        function createOrUpdate(id) {
            var title = "创建渠道"
            if (id) title = "编辑渠道";
            var url = "/shareChannel/toAddOrUpdate?id=" + id;
            bsWindow(url, title, {
        ...
        function deleteById(ids) {
            var option = {
                width: 400, content: "确定要删除？删除之后数据不能再恢复!", confrimFunc: function (jboxObj) {
                    var param = "ids=" + ids;
                    var obj = tools.requestRs("/shareChannel/deleteByIds", param, 'post');
                    if (obj.success) {
                        jboxObj.close();
    ...
    <div id="toolbar" >
        <form class="form-horizontal" id="form1" name="form1">
            <button type="button" class="btn btn-primary" onclick="createOrUpdate(0);">
                <i class="glyphicon glyphicon-plus"></i> 新建渠道
            </button>
            <input type="text" class="form-control" name="name" autocomplete="off" placeholder="输入渠道名称"
