<%
	String contextPath = request.getContextPath();
	String ctx = request.getContextPath();
%>
<link rel="stylesheet" href="${requestContext.contextPath}/web/res/bootstrap/css/bootstrap.min.css">
<link rel="stylesheet" href ="${requestContext.contextPath}/web/res/jbox/css/jBox.css">
<link rel="stylesheet" href ="${requestContext.contextPath}/web/res/jbox/css/jbox-customer.css">
<link rel="stylesheet" href="${requestContext.contextPath}/web/res/style/css/style.css">
<script  src="${requestContext.contextPath}/web/res/jquery/jquery.min.js"></script>
<script src="${requestContext.contextPath}/web/res/bootstrap/js/bootstrap.min.js"></script>
<script src="${requestContext.contextPath}/web/res/jbox/js/jbox.js"></script>
<script src="${requestContext.contextPath}/web/res/js/jboxTools.js"></script>
<script src="${requestContext.contextPath}/web/res/js/sys.js"></script>
<script src="${requestContext.contextPath}/web/res/js/tools.js"></script>
<%--<script src="${requestContext.contextPatg}/web/res/my97/calendar.js"></script>
<script src="${requestContext.contextPatg}/web/res/my97/WdatePicker.js"></script>
<script src="${requestContext.contextPatg}/web/res/my97/xdate.dev.js"></script>--%>
<style>
body{
	font-family: 微软雅黑;
}
</style>
<script>
	var contextPath = "<%=contextPath%>";
</script>
