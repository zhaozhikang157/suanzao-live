...
function queryParams(params) {
    ...
    var cebeginTime = $("#cebeginTime").val();  //起始时间
    var ceendTime = $("#ceendTime").val();    //结束时间

    if (cebeginTime != "")cebeginTime = cebeginTime + " 00:00:00";
    if (ceendTime != "")ceendTime = ceendTime + " 23:59:59";
    ...
    params["cebeginTime"] = cebeginTime;
    params["ceendTime"] = ceendTime;
...
function getCourseDatas(param){
    $.ajax({
        type: 'POST',
        url: '/course/getCourseDataNum',
        data: param,
        dataType: "json",
        success: function (data) {
            if(data.data){
                $(".singleNum").text(data.data.singleNum);
                $(".seriesNum").text(data.data.seriesNum);
                $(".singlePayNum").text(data.data.singlePayNum);
                $(".seriesPayNum").text(data.data.seriesPayNum);
           //     singlePayNum
                var datav = [];
                datav.push(data.data.singleNum);
                datav.push(data.data.seriesNum);
                datav.push(data.data.singlePayNum);
                datav.push(data.data.seriesPayNum);
                //更新数据
                var option = myChart.getOption();
