...
function getStorageSync(key) {
	...
	try {
		...
		console.error('get localStorage failed! ', e);
		return;
	}
	...
	try {
		...
		console.error('parse localstorage data error:', e);
	}
	...
	if (data && 'object' === typeof data && data._expires) {
		...
		if ((+data._expires) < nowDate) {
			try {
				...
				console.error('remove localstorage failed! ',e);
			}
...
function setStorageSync(key, value, expires) {
	...
	try {
		...
		console.error('set localstorage  failed! ', e);
	}
}