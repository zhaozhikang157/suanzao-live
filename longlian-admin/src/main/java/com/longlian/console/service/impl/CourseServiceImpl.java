...
public class CourseServiceImpl implements CourseService {
    ...
    @Override
    public Map getCourseDataNum(Map map) {
        Map resMap=new HashMap<>();
        if("0".equals(map.get("selectDate")+"")){  //当日
            String dateStr = DateUtil.format(new Date(), "yyyy-MM-dd");
            map.put("startTime",dateStr+" 00:00:00");
            map.put("endTime",dateStr+" 23:59:59");
        }else if("1".equals(map.get("selectDate")+"")){//当周
            Map calendarWeek = DateUtil.getCalendarWeek();
            map.put("startTime",calendarWeek.get("mondayDate")+" 00:00:00");
            map.put("endTime",calendarWeek.get("sundayDate")+" 23:59:59");
        }else if("2".equals(map.get("selectDate")+"")){//当月
            Map calendarMonth = DateUtil.getCalendarMonth();
            map.put("startTime",calendarMonth.get("firstday")+" 00:00:00");
            map.put("endTime",calendarMonth.get("lastday")+" 23:59:59");
        }else if("3".equals(map.get("selectDate")+"")) {  //当年
            Map calendarYear = DateUtil.getCalendarYear();
            map.put("startTime",calendarYear.get("firstday")+" 00:00:00");
            map.put("endTime",calendarYear.get("lastday")+" 23:59:59");
        }
        List<Map> maps=courseMapper.getCourseDataNum(map);
        resMap.put("singleNum",maps.get(0).get("totalNum")+"");
        resMap.put("seriesNum",maps.get(1).get("totalNum")+"");
        resMap.put("singlePayNum",maps.get(2).get("totalNum")+"");
        resMap.put("seriesPayNum",maps.get(3).get("totalNum")+"");
        return resMap;
    }

    @Override
    public Map getCourseDatasIncome(Map map) {
        Map resMap=new HashMap<>();
        if("0".equals(map.get("selectDate")+"")){  //当日
            String dateStr = DateUtil.format(new Date(), "yyyy-MM-dd");
            map.put("startTime",dateStr+" 00:00:00");
        ...
        List<Map> maps=courseMapper.getCourseDataIncome(map);
        resMap.put("singleAmount",maps.get(0).get("totalAmount")+"");
        resMap.put("seriesAmount",maps.get(1).get("totalAmount")+"");
        return resMap;
    }
