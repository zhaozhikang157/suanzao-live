...
//		}
	...
	private String getSqlserverPageSql(Page<?> page, StringBuffer sqlBuffer) {
		...
		sb.append("SELECT TOP "+pageSize+" * FROM(SELECT ROW_NUMBER() OVER (ORDER BY (select 0)) AS RowNumber,* FROM ("+sqlBuffer+") tmpSqlServer1) as tmpSqlServer2 ");
		sb.append("WHERE RowNumber >"+pageSize*(pageNo-1));
		return sb.toString();
	}
