...
(function ($) {
    ...
    BootstrapTable.prototype.disabledCheckBy = function (obj) {
        this.disabledCheckBy_(true, obj);
    };
    BootstrapTable.prototype.disabledCheckBy_ = function (checked, obj) {
        if (!obj.hasOwnProperty('field') || !obj.hasOwnProperty('values')) {
            return;
        }
        ...
        $.each(this.options.data, function (index, row) {
            ...
            if ($.inArray(row[obj.field], obj.values) !== -1) {
                var $el = that.$selectItem.filter(':enabled')
                    .filter(sprintf('[data-index="%s"]', index)).prop('disabled', checked);
                // row[that.header.stateField] = checked;
                // rows.push(row);
                //    that.trigger(checked ? 'check' : 'uncheck', row, $el);
            }
    ...
    var allowedMethods = [
        ...
        'check', 'uncheck',
        'checkBy', 'uncheckBy','disabledCheckBy',
        'refresh',
        'resetView',
        'resetWidth',
        'destroy',
