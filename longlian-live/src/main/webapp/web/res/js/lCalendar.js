...
window.lCalendar = (function() {
	...
	MobileCalendar.prototype = {
		...
		bindEvent: function(type) {
			...
			// 此方法设置试看时间取上次设置时间 初始化时试看分插件默认值
			function timeCtrlInit() {
				var d = new Date();
				var trySeeTime = $('#addLook').attr('trySeeTime');//取试看input属性;
				var seconds = 00;
				var minutes = 00;
				if(trySeeTime){
					seconds=trySeeTime%60;//转换秒
					minutes=parseInt(trySeeTime/60%60);//转换分钟
				}
				var e = {
					hh: 00,
					mm: minutes,
			...
			function finishMobileDateTime(e) {
				...
				var lsSrc = (date_yy % passY + _self.minY)+''+ date_mm+date_dd+(time_hh.length < 2 ? "0" : "")+time_hh+(time_mm.length < 2 ? "0" : "")+time_mm;
				function lsdateTime() {
					var date = new Date();
				...
				if(lsdateTime()-lsSrc>5){
					pop({"content": "不能早于当前时间！" , "status": "error", time: '2000'});
					return;
				}
			...
			function finishMobileTime(e) {
				...
				var addLookTime = parseInt(time_mm)*60+parseInt(time_ss);
				console.log( time_mm ,time_ss,addLookTime)
				if(addLookTime>1800){
					pop({"content": "试看时间不能超过30分钟！" , "status": "error", time: '2000'});
					return;
				}
				_self.trigger.value = time_mm + (time_mm.length < 2 ? ":0" : "分")+ time_ss + (time_ss.length < 2 ? ":0" : "秒");
				$("#addLook").attr('trySeeTime',addLookTime)
				closeMobileCalendar(e);
			}
