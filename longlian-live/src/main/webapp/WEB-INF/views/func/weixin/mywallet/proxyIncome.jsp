...
<div id="wrapper" style="background:#FFF; overflow: auto; height:100%;">
    <div id="cont_box" style="padding-bottom: 0">
        <!--代理收益 -->
        <p class="pucmode">
            总收益：<em class="getmoney">${proxyIncome}</em> 枣币
        </p>
...
<script>
    ...
    function Load() {
        if (oBok) {
            ...
            var result = ZAjaxRes({url: "/accountTrack/proxyIncomeDetailsPage.user?offset="+offset});
            if (result.code == "000000") {
                var html = '';
                $.each(result.data, function (i, item) {
                    html +='<div class="mycombxo" id="selected">'+ '<dl> ' +
                            '<dd> ' + '<h4>'+item.userName+'</h4> ' +
                            '<span class="buycy" >时间: '+item.createTime+'</span> ' +
                            '</dd> ' + '</dl> ' +
