...
<div id="wrapper" style="overflow:hidden; height:100%; -webkit-overflow-scrolling: touch;">
  <div id="cont_box"class="new_index" style="background: white;padding-bottom: 0;height: 100%">
    <ul class="freeNav">
      <li class="on">综合排序</li>
      <li>人气</li>
      <li>价格<em><i class="uptriangle"></i><i class="downtriangle"></i></em></li>
    ...
    <div class="freeContbox" style="padding-top: 2.05rem">
      ...
      <ul class="publicUlstyle" id="muchmoney">
      </ul>
...
//  var vConsole = new VConsole();
  ...
  // 价格
  $('#muchmoney').scroll(function(){
    if(!listInit.sortingTime.ajaxInow){return;}
    listScroll($(this),listInit.muchmoney);
  });
  // 开课时间
  ...
  $(".freeNav li").click(function(){
    ...
    if(index == 0){
      ...
      listInit.muchmoney.sc = 1;
      $(".freeNav li").eq(2).find("em").removeClass("on off");
      $(".freeNav li").eq(3).find("em").removeClass("on off");
      ...
      $(".freeNav li").eq(2).find("em").removeClass("on off");
      $(".freeNav li").eq(3).find("em").removeClass("on off");
      listInit.muchmoney.offset = 0;
      $("#muchmoney").html('');
      if(listInit.muchmoney.sc){
        listInit.muchmoney.sc = 0;
  ...
  function loading(json) {
    switch(json.name){
      case 'comprehensive':
        ...
        var result = ZAjaxRes({url: "/course/findRankingListPage", type: "POST",param:{offset:json.offset,sort:0,pageSize:10}});
      ...
      case 'popuLaritys':
        ...
        var result = ZAjaxRes({url: "/course/findRankingListPage", type: "POST",param:{offset:json.offset,sort:1,pageSize:10}});
        if (result.code == "000000") {
          var mycourse = result.data;
          var size = mycourse.length;
      ...
      case 'muchmoney':
        if(!json.ajaxInow){return};
        listInit.muchmoney.ajaxInow = false;
        var result = ZAjaxRes({url: "/course/findRankingListPage", type: "POST",param:{offset:json.offset,sort:3,pageSize:10,sc:json.sc}});
        if (result.code == "000000") {
          var mycourse = result.data;
          var size = mycourse.length;
          $('#muchmoney').append(htmllist(mycourse));

          listInit.muchmoney.offset=$('#muchmoney li').length;
          listInit.muchmoney.ajaxInow = true;

        }else if(result.code == "000110"){
          if($('#muchmoney li').length < 1 && $('#sortingTime .noData').length < 1){
            $('#muchmoney').append('<div class="noData"></div>');
          }
          listInit.muchmoney.ajaxInow = false;
        }else{
          listInit.muchmoney.ajaxInow = true;
      ...
      case 'sortingTime':
        ...
        var result = ZAjaxRes({url: "/course/findRankingListPage", type: "POST",param:{offset:json.offset,sort:2,pageSize:10,sc:json.sc}});
        if (result.code == "000000") {
          var mycourse = result.data;
          var size = mycourse.length;
...
//        var sc = 0;
//        if(cont==0){
//          a= $("#comprehensive li").length;
//          A(a,10,cont,sc)
//        }else if(cont==1){
//          a= $("#popuLaritys li").length;
//          A(a,10,cont,sc)
//        }else if(cont==2){
//          a= $("#muchmoney li").length;
//          if($(".freeNav li").find("em").hasClass("on")){
//            sc=0
//          }else{
//            sc=1
//          }
...
//  var j = 1;
//  var cont=0;
//  function A(offset,pageSize,sort,sc){
//    var param = {offset: offset,pageSize:pageSize,sort:sort,sc:sc};
//    var result = ZAjaxRes({url: "/course/findRankingListPage", type: "post",param: param});
//    if (result.code == "000000") {
//      console.log(result)
...
//        });
//        if(sort=="0"){
//          $("#publicUlstyle1").append(li);
//        }if(sort=="1"){
//          $("#publicUlstyle2").append(li);
//        }if(sort=="2"){
//          $("#publicUlstyle4").append(li);
//        }if(sort=="3"){
//          $("#publicUlstyle3").append(li);
//        }
//
//      })
//      index++;
...
//      }
//    }else if(result.code == "000110"){
//      $('.publicUlstyle').append('<div class="noData"></div>');
//      oBok=false;
//    }else{
//      multiple=false;
//    }
