    ...
    <script>
        function doInit() {
            $('#table-bootstrap').bootstrapTable({
                ...
                url: "/iosPayType/findIosPayTypeInfoPage",
                ...
                columns: [
                    ...
                    {
                        field: 'llReallyAmount',
                        title: '龙链实收金额',
                        align: 'center',
                        valign: 'middle'
                    },
                    {
                        field: 'userReallyAmount',
                        title: '用户实收金额',
                        align: 'center',
                        valign: 'middle'
                    },
                        ...
                        formatter: function (value, row) {
                            if (row.status == "0") {
                                return [
                                    ...
                                    '<a href="javascript:void(0)" onclick="setStatusForbidden(' + row.id + ',' + row.status + ',' + row.type+');">' + '禁用</a>'
                                ].join('');
                            } else if (row.status == "1") {
                                return [
                                    '<a href="javascript:void(0)" onclick="createOrUpdate(' + row.id + ');">' + '编辑</a>&nbsp;&nbsp;&nbsp;',
                                    '<a href="javascript:void(0)" onclick="setStatusForbidden (' + row.id + ',' + row.status + ',' + row.type+');">' + '启用</a>'
                                ].join('');
                            }
        ...
        function createOrUpdate(id) {
            var title = "添加参数"
            if (id) title = "编辑";
            var url = "/iosPayType/toCreateOrUpdate?id=" + id;
            bsWindow(url, title, {
         ...
         * */
        var jboxObj;
        function setStatusForbidden(id, status,type) {
            ...
            var option = {
                width: 400, content: content, confrimFunc: function () {
                    toSetStatusForbidden(id, status,type);
                },
                draggable: 'title',
                closeButton: 'box',
                animation: {
         ...
         * 根据ID设置禁用
         */
        function toSetStatusForbidden(id, status, type) {
            var obj = tools.requestRs("/iosPayType/updateStatus?id=" + id + "&status=" + status + "&type="+type, 'post');
            if (obj.success) {
