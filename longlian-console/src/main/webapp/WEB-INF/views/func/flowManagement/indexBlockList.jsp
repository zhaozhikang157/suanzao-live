    ...
    <script>
        function doInit() {
            $('#table-bootstrap').bootstrapTable({
                ...
                url: "/flowManagementController/getDescribeLiveStreamsBlockList",
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        formatter: function (value, row) {
                            return [
                                '<a href="javascript:void(0)" onclick="recoveryBlacklist(' + row.appName +','+ row.streamName + ');">' + '恢复推流</a>&nbsp;&nbsp;&nbsp;'
                            ].join('');
                        }
                    }
                ]
         ...
         * */
        var jboxObj;
        function recoveryBlacklist(appName,streamName) {
            var option = {
                width: 400, content: "确定要恢复吗？", confrimFunc: function () {
                    var param = {appName: appName,streamName:streamName}
                    var obj = tools.requestRs("/flowManagementController/recoveryBlacklist", param, 'post');
                    if (obj.success) {
                        jbox_notice({content: "成功恢复!", autoClose: 2000, className: "success"});
                        jboxObj.close();
