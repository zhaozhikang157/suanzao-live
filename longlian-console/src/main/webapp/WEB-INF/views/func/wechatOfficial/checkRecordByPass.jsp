...
<head>
  ...
  <script >
      ...
      $('#table-bootstrap').bootstrapTable({
        ...
        columns: [
          ...
          },
          {
            ...
            formatter: function (value, row) {
              return [
                //'<a href="javascript:void(0)" onclick="toEdit(' + row.id+ ',\''+row.appid+'\');" >' + '修改</a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick="updateManager(' + row.id+ ');" >' + '修改负责人</a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick="unBindLiveRoom(' + row.id+ ');" >' + '解绑</a>&nbsp;&nbsp;&nbsp; <a href="javascript:void(0)" onclick="dataRecharge(' + row.id + ','+row.liveId+');" >' + '流量充值</a>'
                '<a href="javascript:void(0)" onclick="toEdit(' + row.id+ ',\''+row.appid+'\');" >' + '修改</a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick="unBindLiveRoom(' + row.id+ ');" >' + '解绑</a>&nbsp;&nbsp;&nbsp; <a href="javascript:void(0)" onclick="dataRecharge(' + row.id + ','+row.liveId+');" >' + '流量充值</a>'
              ].join('');
            }
          }]
      });
    ...
    function toEdit(aid,appId,payAmount) {
        var title = "编辑";
        var url = "/wechatOfficial/toEdit?id=" + aid+"&appId="+appId;
        bsWindow(url, title, {
            height: 300, width: 700, buttons: []
        });
    }
    function closeWindows(obj) {
        if (obj.success) {
            window.BSWINDOW.modal("hide");
            jbox_notice({content: "修改完毕", autoClose: 2000, className: "success"});
            doQuery();
        } else {
            window.BSWINDOW.modal("hide");
            jbox_notice({content: "系统错误", autoClose: 2000, className: "warning"});
      ...
      function updateManager(id) {
        var url = "/wechatOfficial/toUpdateManager?id=" + id;
        bsWindow(url, "编辑负责人", {
            height: 200, width: 300, submit: function (v, h) {
...
<div class="container" style="width:97%;margin-top:10px">
  ...
  <div id="toolbar">
    <form class="form-horizontal" id="form1" name="form1">
      ...
      <label>直播间id:</label>
      <input type="text" class="form-control" name="liveId" id="liveId" autocomplete="off" placeholder="直播间id"
             placeholder="" style="width: 220px;display: inline-block;">

      <label>直播间名称:</label>
      <input type="text" class="form-control" name="liveName" id="liveName" autocomplete="off" placeholder="直播间名称"
             placeholder="" style="width: 220px;display: inline-block;">
