...
public class StatController {
     ...
     * @throws Exception
     */
    @RequestMapping(value = "/importCountExcel")
    @ResponseBody
    public void importCountExcel(@ApiParam(value = "beginTime", required = false) String beginTime,@ApiParam(value = "endTime", required = false) String endTime,HttpServletRequest req,HttpServletResponse response) throws Exception {
        ExportExcelWhaUtil exportExcelWhaUtil = userCountService.importCountExcel(beginTime,endTime,req);
...
//        BigDecimal dayAllPlatformCourseCounts =redisUtil.exists(RedisKey.ll_new_platform_course_key + dateStr)?getCount(String.valueOf(redisUtil.scard(RedisKey.ll_new_platform_course_key + dateStr))):new BigDecimal(0); //平台开课数
...
//        dayMap.put("dayAllPlatformCourseCounts",dayAllPlatformCourseCounts+"");
...
//        redisUtil.del(RedisKey.ll_console_count_key);
        ...
        if (redisUtil.exists(RedisKey.ll_console_count_key+dateStr)) {
           ...
           //redisUtil.del(RedisKey.ll_console_count_key+dateStr);
            ...
            //平台开课数
//            Long yestAllPlatformCourseCounts = userCountService.getCountUser(Yestday, Yestday, Integer.valueOf(CountType.ALL_PLATFORM_COURSE_COUNTS.getType()));
//            Long weekAllPlatformCourseCounts = userCountService.getCountUser(weekLimit[0], now, Integer.valueOf(CountType.ALL_PLATFORM_COURSE_COUNTS.getType()));
//            Long monthAllPlatformCourseCounts = userCountService.getCountUser(monthLimit[0], now, Integer.valueOf(CountType.ALL_PLATFORM_COURSE_COUNTS.getType()));
//            Long yearAllPlatformCourseCounts = userCountService.getCountUser(yearLimit[0], now, Integer.valueOf(CountType.ALL_PLATFORM_COURSE_COUNTS.getType()));
            String dateStart=DateUtil.format(new Date(), "yyyy-MM-dd")+" 00:00:00";
            String dateEnd=DateUtil.format(new Date(), "yyyy-MM-dd")+" 23:59:59";
            Long dayAllPlatformCourseCounts = courseService.getOpenCourseCountsByTime(DateUtil.format(dateStart), DateUtil.format(dateEnd));
            Long yestAllPlatformCourseCounts = courseService.getOpenCourseCountsByTime(DateUtil.format(dateStart), DateUtil.format(dateEnd));
            Long weekAllPlatformCourseCounts = courseService.getOpenCourseCountsByTime(weekLimit[0], now);
            Long monthAllPlatformCourseCounts = courseService.getOpenCourseCountsByTime(monthLimit[0], now);
            Long yearAllPlatformCourseCounts = courseService.getOpenCourseCountsByTime(yearLimit[0], now);
           ...
           if(yearDays!=0){
            ...
            //平台开课数
            map.put("dayAllPlatformCourseCounts", dayAllPlatformCourseCounts+"");
            map.put("yestAllPlatformCourseCounts", yestAllPlatformCourseCounts+"");
            map.put("weekAllPlatformCourseCounts", weekAllPlatformCourseCounts+"");
            map.put("monthAllPlatformCourseCounts", monthAllPlatformCourseCounts+"");
            map.put("yearAllPlatformCourseCounts", yearAllPlatformCourseCounts+"");
