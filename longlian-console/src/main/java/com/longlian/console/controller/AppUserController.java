...
public class AppUserController {
     ...
     * @return
     */
    @RequestMapping(value = "/membershiprebate",method =RequestMethod.POST)
    @ResponseBody
//    public ActResult membershiprebate(long id,@RequestParam(value = "map",required = false)String mapRequet){
    public ActResult membershiprebate(@RequestBody Map map){
        ActResult ar = new ActResult();
        if(map==null){
            map=new HashMap();
        }
        ...
        Map map1 = appUserService.membershiprebate(id,(Map)map.get("timeMap"));
        ar.setData(map1);
        return ar;
    }
