...
public class WechatOfficialController {
    ...
    @RequestMapping(value = "/updateWechatOfficialRoomInfo" ,method = RequestMethod.POST)
    @ResponseBody
    public ActResult updateWechatOfficialRoomInfo(HttpServletRequest request ,Long id,String mobile,Date freeDate,Long manager,BigDecimal payAmount,String appId)throws  Exception{
        ActResult actResult =  new ActResult();
        try {
            WechatOfficialRoom wechatOfficialRoom=new WechatOfficialRoom();
            wechatOfficialRoom.setId(id);
            wechatOfficialRoom.setMobile(mobile);
            wechatOfficialRoom.setFreeDate(freeDate);
            wechatOfficialRoom.setManager(manager);
            if(wechatOfficialRoom!=null && !Utility.isNullorEmpty(wechatOfficialRoom.getManager()) && wechatOfficialRoom.getManager()!=null &&
                    wechatOfficialRoom.getFreeDate()!=null){
                wechatOfficialRoomService.updateManagerInfo(wechatOfficialRoom,payAmount,appId);
                actResult.setSuccess(true);
            }else{
                actResult.setSuccess(false);
                actResult.setMsg("修改失败");
            }
            return actResult;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ActResult unBindLiveRoom(HttpServletRequest request,Long id) throws Exception {
        ...
        //发送提示短信
        String contractMobile = wechatOfficialRoom.getContactMobile();
        if(contractMobile != null && contractMobile.length() > 0){
            String content = wechatOfficialService.getContentByWechat();
            ...
            messageClient.sendMessage(contractMobile,content);
            log.info("公众号解绑 短信通知 已发送；"+contractMobile);
        }else{
            log.error("error 公众号解绑 短信通知 手机号为空！！！");
        }
        return ActResult.success();
    }

    @RequestMapping("/toEdit")
    public ModelAndView toEdit(Long id,String appId) {
        ModelAndView model = new ModelAndView("/func/wechatOfficial/toEdit");
        model.addObject("id", id);
        model.addObject("appId", appId);

        WechatOfficial wechatOfficial=wechatOfficialService.selectByAppid(appId);
        if(wechatOfficial!=null){
            model.addObject("payAmount", wechatOfficial.getPayAmount());
        }
        return model;
    }
}
