...
public class CourseServiceImpl implements CourseService {
    ...
    public List<Map> getCourseOrdersList(DataGridPage requestModel,Map map) {
      //  int startIndex=(requestModel.getCurrentPage()-1)*requestModel.getLimit();
        requestModel.setTotal(Integer.parseInt(courseMapper.getCourseOrdersListTotal(map).get("totalCount")+""));
        map.put("startIndex",requestModel.getOffset());map.put("pageSize",requestModel.getLimit());
        List<Map> courseOrdersList = courseMapper.getCourseOrdersList(requestModel, map);
    ...
    public List<CourseDto> getListPage(DatagridRequestModel requestModel, Map map) {
        ...
        map.put("startIndex",requestModel.getOffset());map.put("pageSize", requestModel.getLimit());
        List<CourseDto> list = courseMapper.getLists(map);
        requestModel.setTotal(Integer.parseInt(courseMapper.getListTotal(map).get("totalCount") + ""));
        if (list.size() > 0) {
            for (CourseDto c : list) {
                ...
                if(c.getTotalCourseAmount()==null ){
                    c.setTotalCourseAmount(new BigDecimal(0));
                }
                if(c.getTotalRelayCourseAmount()==null){
                    c.setTotalRelayCourseAmount(new BigDecimal(0));
                }
                c.setTotalAmount(c.getTotalCourseAmount().add(c.getTotalRelayCourseAmount()));
    ...
    @Override
    public List<CourseDto> getListChannelSimplePage(DatagridRequestModel requestModel, Map map) {
        map.put("startIndex",requestModel.getOffset());map.put("pageSize",requestModel.getLimit());
        List<CourseDto> list = courseMapper.getListChannelSimplePage(requestModel, map);
    ...
    private static void sort(List<CourseDto> data,String sort,String orderType) {
        Collections.sort(data, new Comparator<CourseDto>() {
            public int compare(CourseDto o1, CourseDto o2) {
                BigDecimal totalCourseAmount = new BigDecimal(0);
                BigDecimal totalCourseAmountv = new BigDecimal(0);
    ...
    public void updateDown(long id, long optId , String optName) throws Exception {
        ...
        courseRelayService.updateDeleteStatusByOriCourseId(id,1);
    ...
    @Override
    @UpdateSeriesCourseTime
    public void updateRelayDown(long id, long optId , String optName) throws Exception {
        //根据id查询转播信息
        CourseRelayDto courseRelayDto=courseRelayService.selectById(id);
        if(courseRelayDto!=null && courseRelayDto.getSeriesCourseId()==0){
            //下架转播课
            courseRelayService.updateRelayCourseStatus(id, 1);
            SystemLogUtil.saveSystemLog(LogType.console_course_down.getType(), "1" ,optId , optName,String.valueOf(id) ,"课程：" + courseRelayDto.getLiveTopic() + "已被下架");
            redisUtil.del(RedisKey.course_live_weekly_selection_id);
            //将所有消息设置已读
            appMsgService.updateMsgCourseStatus(id, "1", courseRelayDto.getRoomId());
        }
    }
    @Override
    @UpdateSeriesCourseTime
    ...
    @Override
    @UpdateSeriesCourseTime
    public void updateRelayUp(long id, long optId , String optName) throws Exception {
        //根据id查询转播信息
        CourseRelayDto courseRelayDto=courseRelayService.selectById(id);
        if(courseRelayDto!=null && courseRelayDto.getSeriesCourseId()==0) {
            //上架转播课
            courseRelayService.updateRelayCourseStatus(id, 0);
            SystemLogUtil.saveSystemLog(LogType.console_course_up.getType(), "1", optId, optName, String.valueOf(id), "课程：" + courseRelayDto.getLiveTopic() + "已被上架");
            redisUtil.del(RedisKey.course_live_weekly_selection_id);
            //将所有消息设置已读
            appMsgService.updateMsgCourseStatus(id, "0", courseRelayDto.getRoomId());
        }
    }
    ...
    public void del(long id, long optId, String optName) throws Exception {
        if(id>=SystemCofigConst.RELAY_COURSE_ID_SIZE){
            ...
            CourseRelayDto courseRelayDto=courseRelayService.selectById(id);
            if(courseRelayDto!=null && courseRelayDto.getSeriesCourseId()==0) {
                courseRelayService.updateDeleteStatus(id, 1);
                SystemLogUtil.saveSystemLog(LogType.console_course_del.getType(), "1", optId, optName, String.valueOf(id), "课程：" + courseRelayDto.getLiveTopic() + "已被删除");
                redisUtil.del(RedisKey.course_live_weekly_selection_id);
            }
        }else{
            courseMapper.del(id);
    ...
    public void restore(long id, long optId, String optName) throws Exception {
        if(id>=SystemCofigConst.RELAY_COURSE_ID_SIZE){
            ...
            CourseRelayDto courseRelayDto=courseRelayService.selectById(id);
            if(courseRelayDto!=null && courseRelayDto.getSeriesCourseId()==0) {
                courseRelayService.updateDeleteStatus(id, 0);
                SystemLogUtil.saveSystemLog(LogType.console_course_del.getType(), "1", optId, optName, String.valueOf(id), "课程：" + courseRelayDto.getLiveTopic() + "已被恢复");
            }
        }else {
    ...
    public List<Map> getCourseMessageListPage(DataGridPage page, Long courseId, String liveTopic, String attach) {
        ...
        page.setTotal(courseMapper.getCourseMessageTotal(courseId, liveTopic, attach));
        return listMap;
    }
...
//        }
    ...
    @Override
    public Long getOpenCourseCountsByTime(Date startTime, Date endTime) {
        return courseMapper.getOpenCourseCountsByTime(startTime,endTime);
    }
    ...
    public int updateShowHide(Course course) throws Exception{
        int status = courseMapper.updateShowHide(course);
        return status;
    }
    
    @Override
    public int updateShowRelayHide(Course course){
        int status = courseMapper.updateShowRelayHide(course);
        return status;
    }
}
